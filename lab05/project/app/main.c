...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (MIN_LENGTH_BUFFER_MESSAGE < nrOfChars && readBuffer[3] == BUTTON1_DOWN)
    {
      pressed = LED_ON;
    }
    else if (MIN_LENGTH_BUFFER_MESSAGE < nrOfChars && readBuffer[3] == BUTTON1_UP)
    {
      pressed = LED_OFF;
    }
    CT_LED->BYTE.LED7_0 = pressed;
