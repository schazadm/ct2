...
void TIM2_IRQHandler(void)
{
    ...
    // stop timers if reading is done.
...
static void print_results(void)
{
    ...
    /// STUDENTS: To be programmed
		...
		char str[12];
    sprintf(str, "%d", min_latency);
    pos += 4;
    hal_ct_lcd_write(pos, str);
    pos += 4;

    hal_ct_lcd_write(pos, (char *)label_max);
    sprintf(str, "%d", max_latency);
    pos += 4;
    hal_ct_lcd_write(pos, str);
    
    pos += 4;
    hal_ct_lcd_write(pos, (char *)label_avg);
    pos += 4;
    sprintf(str, "%d", avg_latency);
    hal_ct_lcd_write(pos, str);
    
    pos += 4;
    sprintf(str, "%d", avg_tisr);
    hal_ct_lcd_write(pos, str);
    
    pos += 4;
    hal_ct_lcd_write(pos, (char *)label_load);
    pos += 5;
    sprintf(str, "%d", tim3_interrupt_counter);
    hal_ct_lcd_write(pos, str);
